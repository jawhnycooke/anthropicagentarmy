...
def break_down_objective(objective, file_content=None, previous_results=None, use_search=False):
    ...
    try:
        ...
        opus_response = client.converse(**opus_params)

        response_text = opus_response['output']['message']['content'][0]['text']
        console.print(f"Input Tokens: {opus_response['usage']['inputTokens']}, Output Tokens: {opus_response['usage']['outputTokens']}")
        total_cost = calculate_subagent_cost("anthropic.claude-3-opus-20240229-v1:0", opus_response['usage']['inputTokens'], opus_response['usage']['outputTokens'])
...
def execute_subtask(prompt, search_query=None, previous_haiku_tasks=None, use_search=False, continuation=False):
    """
    Generates a haiku response based on the given prompt.

    Args:
        prompt (str): The prompt for generating the haiku response.
        search_query (str, optional): The search query for performing a QnA search. Defaults to None.
        previous_haiku_tasks (list, optional): A list of previous haiku tasks. Defaults to None.
    ...
    Returns:
        str: The generated haiku response.
    """
    ...
    haiku_response = client.converse(**haiku_params)


    response_text = haiku_response['output']['message']['content'][0]['text']
    console.print(f"Input Tokens: {haiku_response['usage']['inputTokens']}, Output Tokens: {haiku_response['usage']['outputTokens']}")
    total_cost = calculate_subagent_cost("anthropic.claude-3-haiku-20240307-v1:0", haiku_response['usage']['inputTokens'], haiku_response['usage']['outputTokens'])
    console.print(f"Haiku Sub-agent Cost: ${total_cost:.2f}")

    if haiku_response['usage']['outputTokens'] >= 4000:  # Threshold set to 4000 as a precaution
        console.print("[bold yellow]Warning:[/bold yellow] Output may be truncated. Attempting to continue the response.")
        continuation_response_text = execute_subtask(prompt, search_query, previous_haiku_tasks, use_search, continuation=True)
...
def generate_refined_output(objective, sub_task_results, filename, projectname, continuation=False):
    ...
    try:
        ...
        response_text = opus_response['output']['message']['content'][0]['text'].strip()
        console.print(f"Input Tokens: {opus_response['usage']['inputTokens']}, Output Tokens: {opus_response['usage']['outputTokens']}")
        total_cost = calculate_subagent_cost("anthropic.claude-3-opus-20240229-v1:0", opus_response['usage']['inputTokens'], opus_response['usage']['outputTokens'])
        console.print(f"Opus Refine Cost: ${total_cost:.2f}")

        if opus_response['usage']['outputTokens'] >= 4000 and not continuation:  # Threshold set to 4000 as a precaution
            console.print("[bold yellow]Warning:[/bold yellow] Output may be truncated. Attempting to continue the response.")
...
while True:
    ...
    if not task_exchanges:
        ...
        opus_result, file_content_for_haiku, search_query = break_down_objective(objective, file_content, previous_results, use_search)
    else:
        opus_result, _, search_query = break_down_objective(objective, previous_results=previous_results, use_search=use_search)
    ...
    if "The task is complete:" in opus_result:
        ...
        sub_task_result = execute_subtask(sub_task_prompt, search_query, haiku_tasks, use_search)
